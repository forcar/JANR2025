...
//            g2d.setColor(new Color(230,230,230));
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getClickCount() == 2 && !isChild) {
            ...
            double xx = this.getPad(pad).getDatasetPlotters().get(0).getDataSet().getMin();
            double yy = this.getPad(pad).getDatasetPlotters().get(0).getDataSet().getMax();
    ...
    public void setClickedPad(MouseEvent e) {
    	clickedPad = getPadByXY(e.getX(), e.getY());;
    }
    
    public int getClickedPad() {
    	return clickedPad;
    }
...
//        frame.setVisible(true);
    ...
    public void save(String filename, SaveType saveType) {
        ...
        if (saveType == SaveType.TXT) {
            ...
            List<IDataSetPlotter> plotters = this.getPad(popupPad).getDatasetPlotters();
            for (int k = 0; k < plotters.size(); k++) {
                IDataSet data = plotters.get(k).getDataSet();
//            System.out.println(data);
                ((EmbeddedCanvas) data).save(filename + "_" + k + "_" + data.getName() + extension);
            }
        }
