...
public class JanrIniPoint extends Constants {
	...
	public void janrIniPoint(double q2) {
		janrIniHel(q2);
		janrIniRes(q2);
	}
	
    public void janrIniHel(double q2) {
        ...
        aa3[23] = 146 * (1 - 0.42 * q2 + 3.50 * q2 * q2 - 0.0089 * pow(q2, 3)) * exp(-2.5 * q2);
    ...
    public void atomp(double l, double a12, double a32, double s12, double[] mes) {
        ...
        if (l > 0) {
            aa32 = a32 / sqrt(l * (l + 2));
            mes[0] = -2.0 / (2 * l + 2) * (aa32 * (l + 2) + a12);
        } else {
            aa32 = 0.0;
            mes[0] = 0.0;
        }
        mes[1] = 2.0 / (2 * l + 2) * (aa32 * l - a12);
        mes[2] = -sqrt(2.0) * s12 / (l + 1);
    }

    public void atomm(double ll, double a12, double a32, double s12, double[] mes) {
        double l = ll - 1;
        double aa32;
        if (l > 0) {
            aa32 = a32 / sqrt(l * (l + 2));
            mes[1] = -2.0 / (2 * l + 2) * (aa32 * (l + 2) + a12);
        } else {
            aa32 = 0.0;
            mes[1] = 0.0;
        }
        mes[0] = -2.0 / (2 * l + 2) * (aa32 * l - a12);
        mes[2] = -sqrt(2.0) * s12 / (l + 1);
    }
